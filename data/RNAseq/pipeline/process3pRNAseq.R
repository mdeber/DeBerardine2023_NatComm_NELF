...
##        as bedGraph files
## Dependencies include BRGenomics >v0.8; 
##   # install under R >4.0
##   BiocManager::install("BRGenomics")
##   # dev version under R >4.0
...
option_list <- list(
    ...
    make_option(c("-o", "--outdir-raw"),
                type = "character",
                default = getwd(),
                help = "Directory to output raw readcount bedGraph files"),
    make_option(c("-n", "--outdir-norm"),
                type = "character",
                default = getwd(),
                help = "Directory to output spike-in normalized bedGraph files"),
...
msgout(" ")
msgout(paste0("[*] process3pRNAseq v", script.version, " with BRGenomics v", 
              packageVersion("BRGenomics")))
msgout(" ")
msgout(" --- Input Files ---  ")
msgout(" ")
...
dat_list <- import_bam(infiles, mapq = opt$quality,
                       paired_end = FALSE,
                       trim.to = "whole",
...
names(dat_list) <- sub(".bam", "", sub("./", "", basenames))
...
if (spike) {
    ...
    counts.df <- getSpikeInCounts(dat_list, si_pattern = opt$spike_pattern,
                                  ncores = opt$threads)
    
    NFs <- getSpikeInNFs(dat_list, si_pattern = opt$spike_pattern,
                         batch_norm = opt$batch_norm,
                         ctrl_pattern = opt$control_pattern,
                         ncores = opt$threads)
    
    dat_list <- removeSpikeInReads(dat_list, si_pattern = opt$spike_pattern,
                                   ncores = opt$threads)
}
...
export_fun <- function(x, xname, outdir) {
    p_fname <- paste0(outdir, xname, "_plus.bedGraph")
    m_fname <- paste0(outdir, xname, "_minus.bedGraph")
    p_gr <- x[strand(x) == "+"]
    m_gr <- x[strand(x) == "-"]
    score(m_gr) <- -abs(score(m_gr))
    export.bedGraph(p_gr, p_fname)
    export.bedGraph(m_gr, m_fname)
}

## Raw
if (!is.null(opt$outdir_raw)) {
    msgout(" - Exporting unnormalized bedGraph files...")
    invisible(mcMap(export_fun, dat_list, names(dat_list), opt$outdir_raw,
                    mc.cores = opt$threads))
}

if (spike) {
    dat_norm <- applyNFsGRanges(dat_list, NFs, ncores = opt$threads)
    
    if (!is.list(dat_norm))
        dat_norm <- setNames(list(dat_norm), names(dat_list))
    
    ## Normalized
    if (!is.null(opt$outdir_norm)) {
        msgout(" - Exporting spike-in normalized bedGraph files...")
        xnames <- paste0(names(dat_norm), "_spikenorm")
        invisible(mcMap(export_fun, dat_norm, xnames, opt$outdir_norm,
                        mc.cores = opt$threads))
    }
}
...
if (!is.null(opt$outdir_rds)) {
    ...
    saveRDS(dat_list, paste0(opt$outdir_rds, "RNAseq_raw.rds"))
    if (spike)
        saveRDS(dat_norm, paste0(opt$outdir_rds, "RNAseq_norm.rds"))
}
